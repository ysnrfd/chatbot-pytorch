...
# Tokenization and preprocessing of intents
...
def save_data(model, input_size, hidden_size, output_size, all_words, tags, filename):
    data = {
...
def load_data(filename):
    ...
    return data["model_state"], data["input_size"], data["hidden_size"], data["output_size"], data["all_words"], data["tags"]

# Function for predicting class from user input
def predict_class(model, sentence, all_words, tags, stemmer, device):
    model.eval()
    sentence = tokenize(sentence)
    X = bag_of_words(sentence, all_words, stemmer)
...
def bag_of_words(tokenized_sentence, all_words, stemmer):
...
if __name__ == "__main__":
    ...
    # Preprocess intents
    X_train, y_train, all_words, tags = preprocess_intents(intents, stemmer, ignore_words)
    ...
    save_data(model, input_size, hidden_size, output_size, all_words, tags, "data.pth")

    # Load trained model and associated data
    model_state, input_size, hidden_size, output_size, all_words, tags = load_data("data.pth")
    ...
    while True:
        ...
        tag = predict_class(model, sentence, all_words, tags, stemmer, device)
